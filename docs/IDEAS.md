...
| 11 | ✅ **Done, but not this way** — the zigzag is real; `--no-hscroll` is the wrong cure. Rendered, it fixes the alignment by hiding the MATCH — which is exactly backwards for `^] cmd`, the one scope whose whole purpose is matching in that column. `--freeze-left=1` (fzf ≥ 0.67) keeps the identity column pinned *and* the match visible; `--no-hscroll` is kept only as the pre-0.67 fallback. Caveat measured after the fact: the frozen region is right-TRIMMED, so padding is dropped at the cut and the ellipsis lands at a different column on each scrolled row. | S | fzf ≥ 0.67 |
